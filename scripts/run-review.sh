...
7. BRIEFING CONTEXT: read ~/workspace/briefing/stock-portfolio-briefing/content/briefing-${TODAY}.json (if it exists). Extract the macro, moverNotes, and actions fields. Identify any items that are directly relevant to the universe symbols in policy.md or to current open positions. Keep a note of at most 2 relevant observations — these will enrich the digest. If the file does not exist, skip silently.
8. DIGEST: write a notification digest to logs/digest/${TODAY}.md (overwrite if it exists), <= 1600 characters, for a Telegram push. Write it in a natural, conversational tone — like a brief note from a trading assistant to the account owner (Jack), not a machine report. Warm but concise; a little personality is fine; no markdown tables, no robotic headers.
   ...
   Lead with a one-line human summary (e.g. how many buy signals, market context). Weave the numbers into prose or a short bulleted list, but the symbols/percentages/counts must be exactly as computed — never round, invent, or drop them. If the briefing had relevant observations (step 7), add a short "📰 Briefing note:" paragraph at the end — max 2 items, plain prose, no numbers invented. End with a short, non-pushy reminder that nothing is executed automatically. This file is for notification delivery and is NOT committed.
