...
4. Apply guardrails to BUY signals only (5 orders/day max, \$100 each, 20% position cap — the table in config/guardrails.md is the source of truth if these ever disagree). Build the prioritized BUY list per the 'Prioritization (deterministic)' section of policy.md. TRIM signals are not subject to the buy order cap — they are exits. Show the drawdown number for each ranked BUY. Qualitative context goes only in 'Flags', never changes the ranked list.
