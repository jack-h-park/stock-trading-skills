...
3b. REFERENCE — write logs/digest/${TODAY}.md (overwrite if it exists), <= 1600 characters. Everything Jack CANNOT execute from the Agentic account. Open by saying plainly that this message is read-only context.
   ...
   - If the briefing had relevant observations, a short '📰 Briefing note:' — at most 2 relevance-picked items plus the act-now item from step 2b if there was one (so at most 3), plain prose, act-now first. An observation that exists BECAUSE of an Agentic signal belongs in the Agentic message, not here: on 2026-07-31 this note explained what 'drove AAPL's BUY signal in the Agentic account' and what 'triggered its take-profit exit', directly under a heading saying nothing in the message was actionable from that account. Keep here only what stands on its own for the other accounts; if the same news matters to an Agentic proposal, it is the Agentic message that says so.
